...
export async function initSpace(
  ...
  if (baseSpaceRows.length === 0) {
    console.log("Initializing base space", spaceName);
    await insertSpace(executor, spaceName, INITIAL_SPACE_VERSION);
  }
  ...
  for (let i = 0; i < latestData.length; i++) {
    ...
    sampleDataBatchs[sampleDataBatchs.length - 1].push(latestData[i]);
  }
...
export async function getIssueEntries(
  ...
  console.log("getIssueEntries", spaceID);
  const { rows } = await executor(
    `
    SELECT key, value, deleted
    FROM entry
...
`,
  ...
  const filtered: [key: string, value: string][] = rows.map((row) => [
    row.key,
    row.value,
  ]);
...
WITH overlayentry AS(
  ...
  WHERE spaceid = $1 AND key NOT LIKE 'issue/%' AND syncorder > $2
  ORDER BY syncorder LIMIT $3
)
SELECT key, value, syncorder FROM overlayentry 
